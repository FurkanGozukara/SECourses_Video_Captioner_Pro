...
- Trim media, sample video by target FPS, uniform timestamps, keyframes, or adaptive visual change, detect scenes, enforce model duration limits, sub-split with overlap, and optionally reject short, black, static, blurry, or silent clips. Input trims re-encode precisely using the selected re-encode quality; the fast-copy option applies to subsequent clip splitting.
...
- Use protected shipped presets that also cover Chat, apply any preset the moment it is selected, save writable user presets, restore the last-used preset with **Load Last Values**, restore settings from `metadata.json` in Recover Settings, and persist global paths and preferences in `app_settings.json`.
- Select one GPU or multiple data-parallel batch GPUs, apply tier-aware attention plans with automatic decoder block swap that keeps 2 GB of dedicated VRAM free instead of paging into Windows shared GPU memory, recover from supported OOM cases, inspect live VRAM/RAM/shared-memory and model health, and choose default or maximum-autotune `torch.compile` with fallbacks. Both interface choices avoid explicit CUDA graph replay for DynamicCache compatibility.
...
- Tune every backend value from the interface: seed, repetition guard (`no_repeat_ngram_size` and repeated-sentence removal), maximum caption characters and join separator, subtitle cue minimum and line wrapping, context carry words and wrapper prompt, re-encode codec/CRF/preset/audio bitrate (also used by trimming), rejection thresholds (black luma, silence RMS, analysis frames), adaptive sampling sensitivity, total pixel cap, OOM retries and degrade factor, planner slack and pinned RAM budget, the CUDA allocator cap, the summary stage token limit, and the full llama.cpp option set (frames, JPEG quality, threads, batch sizes, flash attention, cache reuse, tier-context bypass, GPU layers, CPU MoE expert layers, min-p, repeat window, presence/frequency penalties, fit headroom, startup/idle timeouts, extra arguments). Every clamp the backend still applies is logged.
- Write exactly one `<name>.txt` per item or clip with **Save only .txt captions** (no JSON, subtitle, JSONL, transcript, reasoning, summary, or `video_caption/` and `audio_caption/` part files), and load custom checkpoints through **Override model loading**: any llama.cpp multimodal GGUF (Q2/Q3 or abliterated Qwen3-Omni conversions, Qwen2.5-VL) or a same-family Transformers folder for the main caption model, plus a Captioner checkpoint or a faster-whisper CTranslate2 folder for the audio caption model.
...
### Plain .txt-only captions

Enable **Save only .txt captions** (Processing Pipeline > 6. Post-processing, or the linked checkbox under **Save outputs next to the source files** in the Caption tab's Folder batch panel) when a trainer only needs one caption file per clip. Every item or produced clip then writes exactly one `<name>.txt` and nothing else: JSON, SRT/WebVTT, JSONL, transcript sidecars, Thinking reasoning files, the long-video summary, and the `video_caption/` and `audio_caption/` part folders are skipped. A merged video + audio caption is still composed into that single file, `metadata.json` and `run_log.txt` remain in the numbered run directory, and batch skip rules look for the single `.txt`. The switch applies to single runs and folder batches alike and is stored in presets and run metadata.
...
| `distil-large-v3.5` | `distil-whisper/distil-large-v3.5-ct2` | 1.52 GB | Distilled English speech recognition |
| `distil-large-v3` | `Systran/faster-distil-whisper-large-v3` | 1.52 GB | Distilled English speech recognition |
| `distil-large-v2` | `Systran/faster-distil-whisper-large-v2` | 1.52 GB | Distilled English speech recognition |
...
## Custom model overrides (experimental)

**Caption > Caption options > Model > Override model loading (experimental)** loads local checkpoints instead of the registered files of the selected variant. Leave a field empty to use the regular model; nothing is downloaded while an override is set.

- **Video / main caption model override**: a `.gguf` file (or a folder containing one) or a Transformers checkpoint folder. The selected Model variant still provides the prompts, media limits, and generation controls; the backend follows the file. A GGUF always runs through the private `llama-server`, so a Q2/Q3 or abliterated Qwen3-Omni conversion, or another llama.cpp multimodal model such as Qwen2.5-VL, works with a Qwen3-Omni Instruct, Thinking, or Captioner variant selected as the base. A Transformers folder must belong to the selected family (`config.json` `model_type` is checked before loading).
- **mmproj file (optional)**: the multimodal projector for a GGUF override. Empty auto-detects an `mmproj*.gguf` beside the model; set it when the projector lives elsewhere or several exist. A GGUF without any projector is text-only.
- **Audio caption model override**: replaces the model behind Processing Pipeline > 8. A Qwen3-Omni Captioner `.gguf` or Transformers folder replaces the sound-caption model; a faster-whisper (CTranslate2) folder containing `model.bin` replaces the Whisper speech model for caption jobs and the Transcribe tab, which shows a note while the override is active. Its own optional mmproj field works like the main one.

The status line under the fields reports what was detected (format, size, projector, family compatibility) or why a path cannot be used, and Start refuses to run with a missing or incompatible override. Override paths are stored in universal presets and in `metadata.json` (`model_info.override` and `model_info.audio_override`), apply to Chat and Caption Editor regeneration, and changing them switches the resident model automatically. llama-server's advertised modalities are honoured: a custom model without an audio encoder is captioned from its sampled frames with a logged warning, and an audio rejection is retried once without audio. Models outside the shipped families (for example Qwen2.5-VL) are not tuned for the shipped prompt presets, so review their captions before a large batch.
...
- Selecting a preset in the universal preset dropdown applies it immediately and exactly as saved; Load re-applies the selected preset after manual edits, and Reset restores application defaults. The automatic VRAM tier plan runs at startup, on Reset, and when a preset selects a variant the detected GPU cannot run (that variant is swapped and the note under the VRAM preset says so); it never overwrites the media, token, or offload values a preset stores. Task / prompt presets in the Caption tab apply on selection as well, by mouse or keyboard.
...
The interface uses the stock Gradio 6 **Origin** theme exactly as shipped, the same theme as the SECourses IndexTTS app, so every colour, radius, shadow, and font comes from Gradio and the page width follows Gradio's own responsive limits. The application stylesheet covers only what a theme cannot express: the multi-hue action buttons (the same recipe as IndexTTS), the app's own markup (file tiles, progress and VRAM meters, find/replace chips, status words), the header rule, preset-strip alignment, confirmation bar, and caption action-row and options-panel layout. Fonts are the ones Gradio bundles, so a page load makes no request to Google Fonts and the app renders identically without internet access.
...
The top-level order is Caption, Processing Pipeline, Transcribe, Chat, Caption Editor, Dataset & Export, Global Settings, Recover Settings, System & Models, and Changelog. Caption keeps file loading and the media preview on the left, with Start, Cancel, and Copy in one row above Output caption on the right. The collapsible Caption options panel sits directly below the output and opens with User prompt followed by System prompt; task/prompt presets, model, and generation controls are in nested sections. Options scroll independently so editing long prompts or expanding advanced controls leaves the output and actions in place. Narrow screens stack the two workspace columns. Additional results contains the JSON, subtitles, reasoning, files, clips, and export tools.
...
| Caption, Processing Pipeline | `Esc` | Arm cancellation for eight seconds; press again to confirm while a caption job is active. |
...
- [QA verification log v1.9.0](docs/QA_VERIFICATION_v1.9.0.md)
