...
tools/
  ...
  bench/benchmark.py             # speed & VRAM table (bf16/int8/int4/gguf) on the test GPU
...
- T1 Core foundation: `vcap/core/{paths,logs,progress,gpu,media,outputs,presets,registry,subprocess_runner}.py` + tests.
- T2 Scene split + preprocess + clip fitness + captions_post + export: `core/{scene_split,preprocess,clip_fitness,captions_post,export}.py` + tests.
- T3 Models layer: registry/base/loader/attention/offload/vram_presets + TimeChat + AVoCaDO + Qwen3-Omni wrappers + downloads bridge.
- T4 Quantization tools: merge_shards, convrot_quantize (int8/int4-W4A8), quant loaders (`models/quant/*`), verify, bench — run on the GPU.
- T5 Pipeline + worker protocol + multi-GPU.
- T6 UI: theme/CSS/components/app + Caption tab.
- T7 UI: Editor tab, Dataset tab, Settings, Recover, Health, Changelog.
- T8 Models_Downloader.py rewrite for our catalog + bat/sh installers + docs.
- T9 Prompt presets + default presets JSON.
