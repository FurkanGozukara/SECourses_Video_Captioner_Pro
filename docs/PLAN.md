...
Worker protocol (subprocess mode): parent spawns `python -u -m vcap.pipeline.worker --gpu 0` once; sends JSON lines `{"cmd":"run_job","spec":{...}}`; worker streams `{"ev":"log"|"progress"|"item"|"result"|"error"}` lines; `{"cmd":"unload"}`, `{"cmd":"ping"}`, `{"cmd":"exit"}`. Keep-model-loaded = keep worker alive; idle timer sends `unload`/`exit`. Cancel = tree kill (psutil children + `taskkill /T /F` / `killpg`) then worker restart on next run. Media prep (ffmpeg/scene detection) runs in the parent process thread pool (CPU) or in the worker before GPU work — decision: run prep in the worker too (single code path), but model loading is deferred until the first caption call so prep never holds VRAM.
